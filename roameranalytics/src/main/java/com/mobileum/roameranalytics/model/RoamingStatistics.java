/**
 * 
 */
package com.mobileum.roameranalytics.model;

/**
 * @author smruti
 *
 */
public class RoamingStatistics {
	
	/**
	 * Contry Code
	 */
	String countryCode;
	
	/**
	 * Total Raomer Count
	 */
	long roamerTotal;
	/**
	 * Silent Raomer Count
	 */
	long roamerSilent;
	
	/**
	 * Premium Raomer Count
	 */
	long roamerPremium;
	
	/**
	 * Value Raomer Count
	 */
	long roamerValue;
	
	/**
	 * mt Count
	 */
	long mt;
	
	/**
	 *	Total MO Count
	 */
	long moTotal;
	/**
	 *	local MO Count
	 */
	long moLocal;
	
	/**
	 * Home Mo Count
	 */
	long moHome;
	
	/**
	 * International Mo Count
	 */
	long  moIntl;
	
	/**
	 * data usage in MB
	 */
	long dataUsage;
	
	/**
	 * Sms count
	 */
	long smsUsage;

	/**
	 * @return the roamerTotal
	 */
	public long getRoamerTotal() {
		return roamerTotal;
	}

	/**
	 * @param roamerTotal the roamerTotal to set
	 */
	public void setRoamerTotal(long roamerTotal) {
		this.roamerTotal = roamerTotal;
	}

	/**
	 * @return the countryCode
	 */
	public String getCountryCode() {
		return countryCode;
	}

	/**
	 * @param countryCode the countryCode to set
	 */
	public void setCountryCode(String countryCode) {
		this.countryCode = countryCode;
	}

	/**
	 * @return the roamerSilent
	 */
	public long getRoamerSilent() {
		return roamerSilent;
	}

	/**
	 * @param roamerSilent the roamerSilent to set
	 */
	public void setRoamerSilent(long roamerSilent) {
		this.roamerSilent = roamerSilent;
	}

	/**
	 * @return the roamerPremium
	 */
	public long getRoamerPremium() {
		return roamerPremium;
	}

	/**
	 * @param roamerPremium the roamerPremium to set
	 */
	public void setRoamerPremium(long roamerPremium) {
		this.roamerPremium = roamerPremium;
	}

	/**
	 * @return the roamerValue
	 */
	public long getRoamerValue() {
		return roamerValue;
	}

	/**
	 * @param roamerValue the roamerValue to set
	 */
	public void setRoamerValue(long roamerValue) {
		this.roamerValue = roamerValue;
	}

	/**
	 * @return the mt
	 */
	public long getMt() {
		return mt;
	}

	/**
	 * @param mt the mt to set
	 */
	public void setMt(long mt) {
		this.mt = mt;
	}

	/**
	 * @return the moTotal
	 */
	public long getMoTotal() {
		return moTotal;
	}

	/**
	 * @param moTotal the moTotal to set
	 */
	public void setMoTotal(long moTotal) {
		this.moTotal = moTotal;
	}

	/**
	 * @return the moLocal
	 */
	public long getMoLocal() {
		return moLocal;
	}

	/**
	 * @param moLocal the moLocal to set
	 */
	public void setMoLocal(long moLocal) {
		this.moLocal = moLocal;
	}

	/**
	 * @return the moHome
	 */
	public long getMoHome() {
		return moHome;
	}

	/**
	 * @param moHome the moHome to set
	 */
	public void setMoHome(long moHome) {
		this.moHome = moHome;
	}

	/**
	 * @return the moIntl
	 */
	public long getMoIntl() {
		return moIntl;
	}

	/**
	 * @param moIntl the moIntl to set
	 */
	public void setMoIntl(long moIntl) {
		this.moIntl = moIntl;
	}

	/**
	 * @return the dataUsage
	 */
	public long getDataUsage() {
		return dataUsage;
	}

	/**
	 * @param dataUsage the dataUsage to set
	 */
	public void setDataUsage(long dataUsage) {
		this.dataUsage = dataUsage;
	}

	/**
	 * @return the smsUsage
	 */
	public long getSmsUsage() {
		return smsUsage;
	}

	/**
	 * @param smsUsage the smsUsage to set
	 */
	public void setSmsUsage(long smsUsage) {
		this.smsUsage = smsUsage;
	}
	
	
	

}
